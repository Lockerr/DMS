  ...
  def self.parse_from_mbr_file
    ...
    (1..counter).each do |i|
      ...
      if book.search('//tr')[i]
        ...
        state = case row[6].inner_text
          when 'Отгружен дилеру' then '1_В наличии'
          when 'Автовоз сформирован' then '2_В Москве'
          when 'Отгружен на свободный склад (Москва)' then '2_В Москве'
          when 'Счёт-фактура выставлена' then '3_Счёт-фактура выставлена'
          when 'Отпуск материала выполнен' then '3_Отпуск материала выполнен'
          when 'Прибыл на склад' then '3_Прибыл на склад'
          when 'Отгружен из Палдиски' then '3_Отгружен из Палдиски'
          when 'Автовоз сформирован (в Москву)' then '3_Автовоз сформирован (в Москву)'
          when 'Прибыл в порт назначения' then '3_Прибыл в порт назначения'
          when 'Доставка в Палдиски' then '3_Доставка в Палдиски'
          when 'А/м создан' then '3_А/м создан'
