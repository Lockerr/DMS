...
function fileQueueError(file, errorCode, message) {
  try {
    if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
      alert("Вы пытаетесь загрузить бслишком много файлов.\n" + (message === 0 ? "Вы превзошли лимит файлов." : "Вы можете загрузить " + (message > 1 ? " до " + message + " файлов." : "one file.")));
      return;
    }
    ...
    switch (errorCode) {
      ...
      case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
        progress.setStatus("Не могу загрузить пустой файл");
  ...
  try {
    ...
    switch (errorCode) {
      ...
      case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
        progress.setStatus("Превышен предел загрузки.");
